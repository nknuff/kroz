...
while 1 == 1:
    ...
    while room == 1:
        ...
        if command.lower() == "east" or command.lower() == 'e':
            ...
            if rock and window:
                ...
                print(
                    "\nThe window is already open. Also how did did get this line of dialog it is logically impossible to get i am proud of u <3.")
        else:
            print("\nNo.")
    # bathroom 2
    while room == 2:
    ...
    while room == 3:
        if room == 3:
            ...
            if hallwayCounter >= 21:
                print(
                    "\nYour vision goes dark as your reality ceases to exist. You have broken the space-time continuum. I suppose this qualifies at escaping?")
                time.sleep(3)
                hallwayCounter = 0
                room = 101
            if hallwayCounter == 5:
            ...
            if not room3 and room != 101:
                print(
                    "You enter a hallway that seems to extend forever in both directions. However there is another door on the other side of the hallway.")
                room3 = True
            elif room3 and room != 101:
                print(
                    "The hallway seemingly extends forever in both directions. There are doors to the east and west however.")
            standby()
        ...
        if room == 4:
            ...
            if not room4:
                if string:
                    ...
                    print(
                        "A single light bulb hanging by a wire illuminates the room. You enter a room that looks more or less like yours. The only difference is the presence of a display case of sorts filled with swords from all around the globe. You can see doors in every direction except east.")
                    room4 = True
            elif room4:
                print(
    ...
    while room == 5:
        if room == 5:
            ...
            if string:
                print(
                    "You stumble through a door and into a small, tight closet. You can feel a string in front of you and an exposed brick wall to your north.")
            if not string:
                print(
                    "You are in a small, tight closet. You can feel a string in front of you and an exposed brick wall to your north.")
            standby()
        if command.lower() == "n" or command.lower() == "north":
            print("\nYou cannot go this way.")
        elif command.lower() == "s" or command.lower() == "south":
    ...
    while room == 6:
        if room == 6:
            ...
            if room6:
                print(
                    "Torches light several passages to the west, east, and south. There is also the ladder you came down on.")
                if not monkey:
                    print("\nOne of the heads, which is shaped like a monkey, seems to emit a strange glow.")
            if not room6:
                print(
                    "You descend down a rickety old ladder into a damp, chilly chamber. The heads of several horrifying creatures are encased in stone and mounted on the walls. Moss and ferns inhabit this narrow chamber, and torches light several passages to the east, west, and south.")
                if not monkey:
                    print("\nOne of the heads, which is shaped like a monkey, seems to emit a strange glow.")
                room6 = True
            standby()
    ...
    while room == 7:
        if room == 7:
            ...
            if not room7:
                print(
                    "You enter a movie theater playing an old black and white film on a screen to the west. As you walk around the empty theater, you get the feeling that someone is watching you. The only door you see is the door you came in from. However, there is an air vent to the north and what appears to be a sewer grate in the floor.")
                room7 = True
            elif room7:
                print(
                    "You still can't shake the feeling that someone is watching you. There is an air vent to the north, a sealed grate in the floor, a door to the east, and a screen to the west.")
            standby()
        if command.lower() == "n" or command.lower() == "north":
            if vent:
    ...
    while room == 8:
        if room == 8:
            ...
            if not room8:
                print(
                    "You squeeze through the whole you made with your razor into a clean, white room. The only item in the room is what looks like a portal on the opposite end.")
            if room8:
    ...
    while room == 9:
        if room == 9:
            ...
            if not room9:
                print(
                    "You enter a small, tight air duct, with barely enough room to crawl through. The duct continues to the north, and you can hear a strange noise coming from that direction.")
                room9 = True
            elif room9:
    ...
    while room == 10:
        if room == 10:
            ...
            if not room10:
                print(
                    "You approach a 4 way intersection in the vent system, with vents leading in every direction. The strange noise is louder now, and is still coming from the north.")
                room10 = True
            elif room10:
                print(
                    "Air ducts head in every direction. The strange noise is louder now, and is still coming from the north.")
            standby()
        if command.lower() == "n" or command.lower() == "north":
            room = 11
        elif command.lower() == "s" or command.lower() == "south":
    ...
    while room == 11:
        if room == 11:
            ...
            if not room11:
                print(
                    "The noise is overpowering now, and as you reach the edge of the duct, you can see that it coming from a massive underground river. The duct ends on the face of the cliff so you are left with two options: Go back the way you came, or jump into the river below.")
                room11 = True
            else:
                print(
                    "The river below seems to be the source of the sound. You can either go back the way you came, or jump into the river below.")
            standby()
    ...
    while room == 12:
        if room == 12:
            ...
            if not room12:
                print(
                    "You are still in an air duct. It seems to go to the west and then turns north. There seems to be a loose panel to the south.")
                room12 = True
            else:
    ...
    while room == 14:
        if room == 14:
            ...
            if not room14:
                if monkey:
                    print(
                        "You enter a room filled with rocks. On top of the rocks sits a monkey that glares at you menacingly. The only exit is to the north.")
                elif not monkey:
                    print(
                        "You enter a room filled with rocks. On top of the rocks sits a monkey that is munching on a banana. The only exit is to the north.")
                room14 = True
            else:
                if monkey:
    ...
    while room == 15:
        if room == 15:
            ...
            if not room15:
                print(
                    "You emerge into a lush greenhouse filled with all types of plants and trees. There is a ladder propped up against one of the trees. There is a vent to the south as well as a door to the east.")
                room15 = True
            else:
                print(
                    "There is a ladder propped up against one of the trees. There is a vent to the south as well as a door to the east.")
            standby()
        if command.lower() == "n" or command.lower() == "north":
            print("\nYou cannot go this way.")
        elif command.lower() == "s" or command.lower() == "south":
    ...
    while room == 16:
        if room == 16:
            ...
            if not room16:
                print(
                    "You emerge from the vent into a room filled with tools. Power saws and drill presses line the walls, and several handheld tools including a sledgehammer are scattered around. The only exit is the vent to the east. I wonder how all these tools got here since there are no doors.")
                room16 = True
            else:
                print(
                    "Several handheld tools including a sledgehammer are scattered around the room. The only exit is the vent to the east.")
            standby()
        if command.lower() == "n" or command.lower() == "north":
            print("\nYou cannot go this way.")
        elif command.lower() == "s" or command.lower() == "south":
    ...
    while room == 17:
        if room == 17:
            ...
            if not room17:
                print(
                    "You climb through the window onto a balcony overlooking a maze. The cool air rushes through your hair and you start to feel free again. There is a ladder that seems to go down into the maze, as well as the window you came through.")
                room17 = True
            else:
                print(
    ...
    while room == 18:
        if room == 18:
            ...
            if not room18:
                print(
                    "You drop into a maze. You cannot reach the ladder you came from, so your only option now is to complete the maze. Good luck!")
                room18 = True
            standby()
        if command.lower() == "n" or command.lower() == "north":
            x = random.random()
    ...
    while room == 19:
        if room == 19:
            ...
            if not room19:
                print(
                    "You jump into the freezing cold river below. You shock of the cold water blinds you for a moment, but when you regain vision you cannot see anything but rapidly moving water. You feel yourself moving west, but unscalable cliff faces surround you. You have one move. Choose wisely.")
                room19 = True
            else:
                print(
                    "Cold water. You feel yourself moving west, but unscalable cliff faces surround you. You have one move. Choose wisely.")
            standby()
        if command.lower() == "n" or command.lower() == "north":
    ...
    while room == 20:
        if room == 20:
            ...
            if not room20:
                print(
                    "You find yourself on the shore of a rushing river located south. To your north is a golden gate embedded in the rocky cliff face with a lock near the handle. As you continue to examine your surroundings, you notice a cave to your east.")
                room20 = True
            else:
    ...
    while room == 21:
        ...
        if command.lower() == "n" or command.lower() == "north":
            ...
            print(
                "\nThe Italian's chants grow louder, louder, louder. Weezer's Buddy Holly persists, and the Italian's chants morph into the melody of Buddy Holly.")
        elif command.lower() == "u" or command.lower() == "up":
            print(
            ...
            print(
                "\nYou can't go down. There is no down. This room is made of steel, and you are trapped inside. Just you, and those weird white dudes that wrote Say It Ain't So. This is your life now.")
        elif command.lower() == "e" or command.lower() == "east":
            print(
    ...
    while room == 22:
        if room == 22:
            ...
            if not room22:
                print(
                    "You enter into a grand marble hall with artistic masterpieces hanging on the walls. Most of the paintings have been beaten and battered by vandals with exceptional taste. There is an exposed brick wall to the south, as well as entrances to the east and west.")
                room22 = True
            else:
                print(
                    "Art is here. There is an exposed brick wall to the south, as well as entrances to the east and west.")
            standby()
        if command.lower() == "n" or command.lower() == "north":
            print("\nYou cannot go this way.")
        elif command.lower() == "s" or command.lower() == "south":
    ...
    while room == 23:
        if room == 23:
            ...
            if not room23:
                print(
                    "Grand marble columns line the hall along with several marble statues of various people and mythical creatures. There are passageways to the west and a large door marked EXIT to the east. As you walk through the hall, you feel that gaze again. This time its is stronger. Closer.")
                room23 = True
            else:
                print(
                    "The carcass of the Basilisk lies cold on the floor. There are passageways to the west and a large door marked EXIT to the east.")
            standby()
        if command.lower() == "n" or command.lower() == "north":
            print("\nYou cannot go this way.")
        elif command.lower() == "s" or command.lower() == "south":
    ...
    while room == 24:
        if room == 24:
            ...
            if not room24:
                print(
                    "You open the heavy wooden doors to reveal a monstrous throne room. As you enter the room, the doors slam close behind you and you hear them lock in place. The vaulted ceilings go higher than the eye can see, and there is a throne in the middle of the room. There is a shadowy figure sitting in the chair. They beckon you to approach.")
                room24 = True
    ...
    while room == 25:
        if room == 25:
            ...
            if not room25:
                print(
                    "\nYou somehow manage to escape the maze and enter a room with stone pillars so high you cannot see the room. All you see are stars above you. As a reward for completing the maze, I have chose to give you an extra life. Spend it well.\nThere is a passage to the north.")
                room25 = True
                elife = True
            else:
                print(
    ...
    while room == 27:
        if room == 27:
            ...
            if not room27:
                print(
                    "\nYou walk into a small but cozy cavern. From the looks of things, somebody is living here. Before you process that thought, a small man drops from the ceiling shrieking on his way down. He lands on your back forcing you to the ground. He holds a sharp pointy rock to your throat and demands an entry fee.")
                room27 = True
                ...
                if caveCommand.lower() == "gem":
                    if gem:
                        ...
                        caveman = False
                        print(
                            "\nThe caveman takes your gem and studies it curiously. He seems to be satisfied as he scurries off with his newfound rock making strange grunting noises.")
                    else:
                        print(
                            "\nThe caveman is disappointed that you don't have what you say you have. He calls you a dirty little liar and cuts you into a million pieces, throws them in a big pot, and makes a nice stew for dinner.")
                    youdied()
                else:
                    print(
        ...
        if command.lower() == "n" or command.lower() == "north":
            ...
            if not caveman:
                print(
                    "\nYou pickup the crowbar quietly from the ground. You should probably leave before you the caveman gets mad.")
                crowbar = True
            else:
    ...
    while room == 28:
        if room == 28:
            ...
            if not room28:
                print(
                    "\nYou enter through a veil of frost to reveal an icy room with dangerous looking icicles hanging precariously from the ceiling. There is a stack of what appears to be geodes on the far side of the room, and a heater to your left. The only exit is the way you came in.")
                room28 = True
            else:
                if icicles:
                    if not geode:
                        print(
                            "\nYou enter a room with icicles hanging precariously from the ceiling. There is a stack of what appears to be geodes on the far side of the room, and a heater to your left. The only exit is the way you came in.")
                    else:
                        print(
                            "\nYou enter a room with icicles hanging precariously from the ceiling. There is a heater to your left. The only exit is the way you came in.")
                else:
                    if not geode:
                        print(
        ...
        if command.lower() == "n" or command.lower() == "north":
            if icicles:
                print(
                    "\nYou walk under the icicles and instantly realize your mistake. The vibrations caused by your movement make the sharp daggers of ice fall from the ceiling, instantly killing you.")
                youdied()
            else:
                print("\nYou cannot go this way.")
        elif command.lower() == "s" or command.lower() == "south":
            ...
            print(
                "\nYou switch on the heater and instantly feel its effect. The ice that once coated the walls and the icicles on the ceiling instantly melt leaving nothing to fear.")
            icicles = False
